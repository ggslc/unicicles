...
def regrid_one_stage_to_NEMO(field_CF,nemo_ncgridfile):
  ...
  #in python3 cf masking on f is now screwing up the regrid
  f1=np.asarray(field_CF.array)
  field_CF.set_data(cf.Data(f1))

  g = cf.read(nemo_ncgridfile)[0]

  #in python 3 cf field CF is not identifying axes in a way regrids can identify. 
  #AS above, both axis and aux_coord are identifying as 'X' Add src_axes=['latitude','longitude']
...
def reroute_calving_to_ocean(calv_cf,route_file,coupling_period,ice_tstep_multiply,orig_calving,output_file):
  ...
  print("rere2")
  print(np.sum(new_seed), np.max(new_seed), np.min(new_seed))
  print()
