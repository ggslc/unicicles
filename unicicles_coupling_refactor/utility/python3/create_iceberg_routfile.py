...
if __name__ == "__main__":
  ...
  if res == "eORCA025":
    ...
    print("WARNING: ORCA resolution of input meshmask not recognised - mapping whole domain")
    s_lim = ny
    n_lim = 0
    e_lim = 0 # not sure e and w are the right way round?
    w_lim = nx
    #exit()
  ...
  outfield = outfile.createVariable('lon',float,('y','x'))
  ...
  outfield[:] = Dataset(bathyfile).variables['glamt'][:].squeeze()
  
  outfield = outfile.createVariable('lat',float,('y','x'))
  ...
  outfield[:] = Dataset(bathyfile).variables['gphit'][:].squeeze()
  
  outfield = outfile.createVariable('xindex',float,('y','x'))
  outfield.setncattr('long_name','x index of nearest wet surface cell to ice')
  outfield[:] = x_index
  
  outfield = outfile.createVariable('yindex',float,('y','x'))
