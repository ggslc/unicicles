...
def parse_commandline(argv):
  ...
  parser.add_argument("--fix_polarrow_landfrac", help="experimental, weird AIS polar row land frac artefacts")
  ...
  if args.fix_polarrow_landfrac != None:
      fix_polarrow_landfrac = args.fix_polarrow_landfrac
  else:
    fix_polarrow_landfrac = None

  return um_input,um_ref_input,toice_input,fromice_input,fromcap_input,um_output, tonextice_output, coupling_period, do_snow_shuffling, new_landseamask, fix_polarrow_landfrac

##process filenames etc from the command line
if __name__ == "__main__":
  um_input, um_ref_input, toice_input, fromice_input, fromcap_input, um_output, tonextice_output, coupling_period, do_snow_shuffling, new_landseamask, fix_polarrow_landfrac = parse_commandline(sys.argv)
  ...
  if do_snow_shuffling == True:
    ...
    if np.sum(smb_from_nisnow) > 0.0 :
      ...
      for prop in ['stash_code','um_stash_source']:
        if g.has_property(prop): g.del_property(prop)

      g.set_data(cf.Data(smb_from_nisnow/coupling_period, units='kg/m2/s'))
