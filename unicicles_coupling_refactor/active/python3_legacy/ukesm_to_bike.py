    ...
    if select_field.shape[0] < 50:
      if select_field.shape[0] == 9: #elevated tiles not on. Make a one-way coupling field
        ...
        for attr in ['long_name','stash_code','um_stash_source','_FillValue']:
          dummy.set_property(attr,select_field.getprop(attr))

        dummy.set_construct(select_field.dim('dim0'))
        dummy.set_construct(select_field.dim('dim2'))
        dummy.set_construct(select_field.dim('dim3'))

        fakeelevs=cf.DimensionCoordinate()
        fakeelevs.set_property('long_name','pseudolevel')
        if code == 8236:
          fakeelevs.set_data(cf.Data(np.arange(nelev)+926))
          array = select_field.subspace[select_field.coord('long_name=pseudolevel') == 8, ...].array.squeeze()
        else:
          fakeelevs.set_data(cf.Data(np.arange(nelev)+901))
        ...
        dummy.set_construct(fakeelevs)
        dummy.set_data(cf.Data(fakearray))
...
def calculate_area(field_list):
    ...
    for attr in ['standard_name','stash_code','um_stash_source']:
      if tile_area.has_property(attr): tile_area.del_property(attr)
...
def change_names(out_list):
  ...
  field.coord('long_name=pseudolevel').set_property('standard_name','tile_id')
...
def aggregate_tiles(tile_field,field_list,scaling_method=0):
  ...
  tile_field.set_data(cf.Data(tile_array))
...
#to do Edwards lapse rate we need: current orog. orog of the levels. Identification of +/- value. Identification of latitude.
  ...
  tile_field.set_data(cf.Data(tile_array_new))
