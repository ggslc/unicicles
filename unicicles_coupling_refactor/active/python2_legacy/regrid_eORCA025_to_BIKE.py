...
#melt_water=h.select('long_name:Ice shelf melting')[0].array.squeeze()
melt_water=h.select_field('ice_shelf_melting').array.squeeze()
#melt_heat =h.select('long_name:Ice shelf heat content flux')[0].array.squeeze()
...
print " regrid 2"
f=cf.read(nemo_ncgridfile)[0]
f.insert_data(cf.Data(melt_heat,units='kg/m2/s'), axes=('Y', 'X'))
