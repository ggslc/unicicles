...
!            call glimmer_read_restart_gcm(instance%model, gcm_restart_file)
    ...
    if (instance%model%options%whichdycore == DYCORE_GLIDE) then  ! SIA dycore
      ...
      !use glide memory layout, for now (TODO may need to change temperature)
      call glide_initialise(instance%model)
      !start up bisicles
      call bisicles_initialise(instance%model)
      !initialize the state, either from scrtach or a from restart
      call bisicles_init_state_diagnostic(instance%model, 0) !not sure what the timestamp should be in this case
...
!!    deallocate(thk)
  ...
  subroutine glint_i_initialise_gcm(config,           instance,         &
    ...
    integer :: config_fileunit, restart_fileunit, restart_timestamp
...
!            call glimmer_read_restart_gcm(instance%model, gcm_restart_file)
            instance%model%options%is_restart = 1
            print*,'reading a restartcalled', gcm_restart_file
...
!WHL - Do not call this if comparing to oldglide (cism1) results
      ...
      !use glide memory layout for now (TODO may need to modify temperature)
      call glide_initialise(instance%model)
      !start up bisicles
      call bisicles_initialise(instance%model)
      !initialise the state, either from scrtach or a from restart
      call bisicles_init_state_diagnostic(instance%model, um_time)
    ...
    ! fill dimension variables
    call glide_nc_fillall(instance%model)
    ! this fills the glint output which is on one level, so needs the flag
    call glide_nc_fillall(instance%model, outfiles=instance%out_first,vertical_level_flag=.false.) 
