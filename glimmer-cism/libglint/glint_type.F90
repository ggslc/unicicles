...
!       Maybe we should create a new derived type (glint_instance_gcm?) without the extra information.
  ...
  type glint_instance
     ...
     logical :: gcm_restart ! this to check whether we want to do a restart for each instance
     character*20 :: gcm_restart_name
...
contains
  ...
  subroutine glint_i_readconfig(instance,config)
    ...
    call GetSection(config,section,'restart')
    if (associated(section)) then
       call GetValue(section,'restart',instance%gcm_restart)
       call GetValue(section,'restart_name',instance%gcm_restart_name)
    endif

    instance%gcm_restart = .true.
