...
contains
  ...
  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  subroutine glint_snow_ice_conversion_gcm( instance, sdep_l, wrk, timestep)
    ...
    real(sp), dimension(:,:) ,intent(inout) :: wrk ! work space
    ...
    where (.not.GLIDE_IS_OCEAN(instance%model%geometry%thkmask) & 
         .and. sdep_l > depth_snow2ice .and. wrk < depth_snow2ice )
       !snow -> ice 
       wrk = sdep_l
       sdep_l = - sdep_l
    elsewhere (.not.GLIDE_IS_OCEAN(instance%model%geometry%thkmask) & 
         .and. wrk < depth_snow2ice )
       ! ice -> snow
       sdep_l = wrk
       wrk = - sdep_l
    elsewhere
    ...
    instance%acab = instance%acab + wrk/timestep
    ...
    if (time - instance%mbal_accum%start_time + instance%mbal_tstep == instance%mbal_accum_time) then
       ...
       do i = 1, instance%n_icetstep
          ...
          if (present(sdep_g)) then
             ...
             !     does not work with BISICLES, because BISICLES owns the thickness. 
             !     This version modifies the accumulation
             call glint_snow_ice_conversion_gcm( instance, sdep_l, thck_temp, instance%model%numerics%tinc)
          end if
