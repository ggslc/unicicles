...
!
  ...
  subroutine mean_to_global_mec(ups,                &
    ...
    logical, intent(in),optional :: colfrac ! normalise output as a fraction of column total
    ...
    if (main_task) then
       ...
       do i = 1, nxg
         gnumcol=0
         do n = 1, nec
          gnumcol=gnumcol+gnumloc(i,j,n)
         enddo
         if (gnumcol /= 0) then
          do n = 1, nec
            global(i,j,n) = global(i,j,n) / float(gnumcol)
          enddo
         else
          do n = 1, nec
            global(i,j,n) =0._dp
