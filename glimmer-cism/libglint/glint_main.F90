...
!    endif
  ...
  subroutine initialise_glint_gcm(params,                         &
    ...
    real(rk),               optional,intent(out)   :: ice_volume  !*FD initial ice volume
    real(rk),dimension(:,:,:),optional,intent(out) :: glfrac      !*FD land area fraction on each NEC
    ...
    real(rk),dimension(:,:,:),allocatable ::   &
               gfrac_temp, gtopo_temp, grofi_temp, grofl_temp, ghflx_temp, glfrac_temp    ! Temporary output arrays

    real(sp),dimension(:,:),pointer :: thck_temp    => null()
...
!!       end if
    ...
    allocate(glfrac_temp(params%g_grid%nx, params%g_grid%ny, params%g_grid%nec))
    ...
    do i = 1, params%ninstances
       ...
       if (main_task) then
          if (present(ice_volume)) then
             call coordsystem_allocate(params%instances(i)%lgrid, thck_temp)
             call glide_get_thk(params%instances(i)%model,thck_temp)
             ice_volume=ice_volume + ( sum(thck_temp) * &
                  params%instances(i)%lgrid%delta%pt(1)* &
                  params%instances(i)%lgrid%delta%pt(2) )
             end if
       end if
       ...
       call get_i_upscaled_fields_gcm(params%instances(i), params%g_grid%nec,  &
                                      ...
                                      ghflx_temp,          glfrac_temp)

       ! Splice together with the global output

        if (present(gfrac))                          &
           call splice_fields_gcm(gfrac_temp, gfrac, &
           params%g_grid%nec, params%instances(i)%frac_coverage, params%cov_normalise)

        if (present(gtopo))                          &
           call splice_fields_gcm(gtopo_temp, gtopo, &
           params%g_grid%nec, params%instances(i)%frac_coverage, params%cov_normalise)

        if (present(grofi))                          &
           call splice_fields_gcm(grofi_temp, grofi, &
           params%g_grid%nec, params%instances(i)%frac_coverage, params%cov_normalise)

        if (present(grofl))                          &
           call splice_fields_gcm(grofl_temp, grofl, &
           params%g_grid%nec, params%instances(i)%frac_coverage, params%cov_normalise)

        if (present(ghflx))                          &
           call splice_fields_gcm(ghflx_temp, ghflx, &
           params%g_grid%nec, params%instances(i)%frac_coverage, params%cov_normalise)

        if (present(glfrac))                           &
           call splice_fields_gcm(glfrac_temp, glfrac, &
           params%g_grid%nec, params%instances(i)%frac_coverage, params%cov_normalise)
...
!       write (stdout,*) 'tstep_mbal =', params%tstep_mbal
  ...
  subroutine glint_gcm(params,         time,            &
    ...
    real(rk),dimension(:,:,:),optional,intent(in)    :: gareas        ! areas of land on GCM NEC grid - for conservation purposes
    real(rk),dimension(:,:,:),optional,intent(inout) :: gsdep         ! nonice GCM NEC snowdepth field(in), later anomalies(out)
    real(rk),                 optional,intent(out)   :: ice_volume    ! domain ice volume, for overall conservation calculations
    real(rk),dimension(:,:),  optional,intent(out)   :: gcalv         ! iceberg calving flux on GCM grid
    real(rk),dimension(:,:,:),optional,intent(out)   :: glfrac        ! output land area fraction in a NEC
    ...
    real(rk), dimension(:,:,:), allocatable ::   &
       ...
       glfrac_temp   ,&! glfrac for a single instance
       gsdep_temp      ! gsdep for a single instance
...
!       write (stdout,*) 'tstep_mbal =', params%tstep_mbal
    ...
    print*,'times from glint_main',time, params%next_av_start
    ...
    if (time - params%av_start_time + params%time_step > params%tstep_mbal) then
       ...
       allocate(glfrac_temp(params%g_grid%nx, params%g_grid%ny, params%g_grid%nec))
       !if (present(gsdep)) 
       allocate(gsdep_temp(params%g_grid%nx, params%g_grid%ny, params%g_grid%nec))
       !if (present(gcalv)) 
       allocate(gcalv_temp(params%g_grid%nx, params%g_grid%ny))
       ...
       if (present(ghflx)) ghflx = 0.0
       if (present(glfrac)) glfrac = 0.0
       if (present(gcalv)) gcalv = 0.0
       if (present(ice_volume)) ice_volume = 0.0
       if (present(gsdep)) then 
         gsdep_temp=gsdep
         !we want to return gsdep as an /anomaly/ field for the UM to deal
         !with,not update it all in here
         gsdep=0.
       endif
...
!                write (stdout,*) 'g_av_qsmb (kg/m2/s) =', params%g_av_qsmb(i,j,n)
       ...
       do i = 1, params%ninstances
          ...
          if (present(gsdep)) then
            allocate(lsdep_temp(params%instances(i)%lgrid%size%pt(1),params%instances(i)%lgrid%size%pt(2)))
            lsdep_temp(:,:)=0.
          endif 
          ...
          !Will adjust for conservation in GCM, no need to splice
          if (present(gcalv)) gcalv=gcalv+gcalv_temp
          ...
          !Will adjust for areas in GCM, no need to splice
          if (present(gsdep)) gsdep=gsdep+gsdep_temp
...
! BISICLES restarting 
  ...
  subroutine glim_force_write_restart(params,timestamp)

    ! this will force a writing of the current state of the ice sheet.
#ifdef BISICLES_CDRIVER
    use bisicles_cdriver ! only: bisicles_force_write_restart

    type(glint_params),intent(inout) :: params          ! parameters for this run

    integer :: i
    integer :: timestamp ! this comes from the UM and is currently an integer

    do i = 1, params%ninstances

       print*,'write a restart', timestamp

       call bisicles_checkpoint(params%instances(i)%model,timestamp)
       
       ! call the restart writing algorithm 
 
    enddo

#endif
  end subroutine glim_force_write_restart
...
! ====================================================
  ...
  subroutine splice_fields_gcm(field_temp, field,         &
                               nec,                       &
                               frac_coverage,             &
                               cov_normalise)
     ...
     real(rk), dimension(:,:,:), intent(in) :: field_temp  ! output fields for this instance

     real(rk), dimension(:,:,:), intent(inout) :: field    ! spliced global output field
