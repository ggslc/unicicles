...
!        replace some work types (tempwk, velowk) with local arrays and parameters.
  ...
  integer, parameter :: DYCORE_BISICLES_CDRIVER = 3 ! BISICLES through the cdriver interface, rather than the external dycore interface
#endif
  ...
  integer, parameter :: TEMP_BC_TEMPERATURE = 0 !*FD Temperature (Dirichlett) boundary coindition
  integer, parameter :: TEMP_BC_FLUX = 1 !*FD Flux (Neumann) boundary coindition
...
#ifdef BISICLES_CDRIVER
    !id of BISCLES wrapper stored on the C++ side
    integer :: bisicles_instance_id
    logical :: bisicles_restart = .false.
    logical :: bisicles_restart_override = .false.
    character(len=128) :: bisicles_chk_fmt ="('chk.'I0.9'.2d.hdf5')"
#endif
...
!TODO - Change to dp
  ...
  type glide_climate
     ...
     real(sp),dimension(:,:),pointer :: hflx     => null() !*FD Annual heat flux into ice at upper surface
...
!!     real(sp) :: slidconst = 0.0     ! not currently used
     integer :: temp_upper_bc_type =  TEMP_BC_TEMPERATURE !*FD type of boundary condition for the heat equaton at the upper surface 
...
!      - In the glam/glissade/bisicles dycores, temperature and flow factor live on the 
...
!        is defined at the upper surface (k = 0) and lower surface (k = upn).
    ...
    else   ! glam/glissade/bisicles dycore
...
!    call coordsystem_allocate(model%general%velo_grid, upn, model%velocity%magres)
    ...
    call coordsystem_allocate(model%general%ice_grid, model%climate%hflx)
...
!    deallocate(model%velocity%magres)
    ...
    if (associated(model%climate%hflx)) &
         deallocate(model%climate%hflx)
