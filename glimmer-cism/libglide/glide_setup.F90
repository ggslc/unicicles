...
!--------------------------------------------------------------------------------
  ...
  subroutine handle_options(section, model)
    ...
    ! this is what sets a bisicles restart 
    call GetValue(section,'bisicles_restart',model%options%bisicles_restart)
    !ignore conflict between bisicles time and driver time
    call GetValue(section,'bisicles_restart_override',model%options%bisicles_restart_override)
    !format for the BISICLES restart file
    !call GetValue(section,'bisicles_checkpoint_format',model%options%bisicles_chk_fmt)
    ! this is what sets a bisicles restart 
...
!--------------------------------------------------------------------------------
  ...
  subroutine print_options(model)
    ...
    character(len=*), dimension(0:3), parameter :: dycore = (/ &
         ...
         'bisicles_cdriver   ' /)  ! bisicles cdriver interface
...
#ifdef BISICLES_CDRIVER
    else if (model%options%whichdycore == DYCORE_BISICLES_CDRIVER) then

       !had to put this in to avoid death at the next else 
       call write_log('Warning, bisicles dycore reads its options from its own input file')

       write(message,*) 'the bisicles restart status is: ', model%options%bisicles_restart
       call write_log(message)

       write(message,*) 'the bisicles restart override is: ', model%options%bisicles_restart_override 
       call write_log(message)
...
!!    call write_log(message)
    ...
    if ((model%options%whichdycore == DYCORE_GLAM) & 
         .or. (model%options%whichdycore == DYCORE_GLISSADE)) then   ! glam/glissade higher-order
