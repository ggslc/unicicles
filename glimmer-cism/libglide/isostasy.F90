...
!-------------------------------------------------------------------------
  ...
  subroutine isos_compute(model)
    ...
    !store old bedrock topography in model%isostasy%gia, to later compute difference
    model%isostasy%gia = model%geometry%topg
    ...
    !compute gia = db/dt given change in topography (b)
    model%isostasy%gia = (model%geometry%topg -  model%isostasy%gia)/model%numerics%dt
