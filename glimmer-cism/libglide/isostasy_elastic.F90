...
!-------------------------------------------------------------------------
  ...
  subroutine init_elastic(rbel, deltax)
    ...
    if (main_task) then
       ...
       close(1)
#endif
    end if ! end if (main_task)
    !rbel%w=rbel%w/len0

  end subroutine init_elastic
!-------------------------------------------------------------------------

  subroutine calc_elastic_serial(rbel,load,load_factors)
  ...
  end subroutine calc_elastic_serial
...
!-------------------------------------------------------------------------
  ...
  subroutine calc_elastic(rbel,load,load_factors)
    ...
    !*FD (not-really-)parallel wrapper around calc_elastic_serial
    !*FD ie we simply gather load_factors and load onto one
    !*FD rank, do the calculation, and scatter back
    !*FD (alternative would be to use a big halo)
    use parallel, only : main_task, distributed_scatter_var, & 
         distributed_gather_var, global_ewn,global_nsn

    implicit none
    type(isos_elastic) :: rbel                     !*FD structure holding elastic litho data
    real(dp), dimension(:,:), intent(out) :: load !*FD loading effect due to load_factors
    real(dp), dimension(:,:), intent(in)  :: load_factors !*FD load mass divided by mantle density
    real(dp), dimension(:,:), allocatable :: load_fulldomain, load_factors_fulldomain
   
    write(*,*) 'gather...'
    call distributed_gather_var(load_factors, load_factors_fulldomain)
    write(*,*) '...done'
    if (main_task) then
       allocate(load_fulldomain(global_ewn,global_nsn))
       call calc_elastic_serial(rbel,load_fulldomain,load_factors_fulldomain)
    else
       !allocation on all tasks because distributed_scatter_var likes to de-allocate...
       allocate(load_fulldomain(0,0))
    end if
       
    if (allocated(load_factors_fulldomain)) then
       deallocate(load_factors_fulldomain)
    end if
    write(*,*) 'scatter...'
    call distributed_scatter_var(load, load_fulldomain)
    write(*,*) '...done'
